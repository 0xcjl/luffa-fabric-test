...
export const projectDocsStatus = [
  ...
  { label: "Last Updated", value: "2026-06-15" },
...
export const projectDocsSections: ProjectDocSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "Endless transfer：浏览器真实 txHash 优先走官方 Endless Web Wallet SDK；Luffa App QR / WebView 继续承担 luffa-endless-auth:v1 原生授权协议验收。",
      "Luffa App QR / WebView transfer：真实 App callback 必须带 publicKey、fullMessage、signature 并通过验签；若 App bridge 的 packageTransactionV2 返回空 rawData 或 1006/1009，归类为 App bridge payload 兼容问题。",
  ...
  },
  {
    ...
    items: [
      ...
      "执行 Endless Web Wallet 转账 / Reward：选择 Endless Testnet 或 Endless Mainnet，右上角 Wallet / Network 点击 Use Endless Web Wallet，生成 proposal 后点击 Sign Endless Web Wallet Tx，拿到真实 txHash 后再 Approve & Record。",
  ...
  },
  {
    ...
    items: [
      ...
      "当前真实链上执行仍以测试网 / devnet 为主；主网可连接和生成 proposal，但真实签名执行默认禁用，Base Mainnet 小额实测必须显式开启安全门。",
  ...
  },
  {
    ...
    items: [
      ...
      "Endless Web Wallet Session Report 记录 2026-06-15 Web Wallet SDK 路径、wallet binding/publicKey 修复、Task Reward 0.001 EDS proposal、当前服务状态和真实 txHash 阻塞项。",
...
export const projectDocsIndex: ProjectDocIndexItem[] = [
  ...
  },
  {
    ...
    note: "记录 Endless Web Wallet SDK 集成、address/publicKey 验签修复、Task Reward 0.001 EDS proposal、当前服务状态和 Confirm 灰色导致 txHash 未完成的阻塞。",
  },
  {
